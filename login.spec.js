...
test('login page', async ({ page }) => {
  ...
  await page.fill("input[id='register.firstName']", 'Najeba');
  ...
  await page.click('//li[contains(text(), "Female")]');
...
//   await expect(genderOptions).toHaveCount(2);
  ...
  await page.fill("input[id='register.email']", 'nmmmohamm55665@g.com');
  await page.fill('#register_password', 'test123');
  await page.fill("input[id='register.checkPwd']", 'test123');
  ...
  await expect(await page.locator("(//div[@class='c_nav-main-bar-link-details'])[3]")).toContainText('Najeba!')
